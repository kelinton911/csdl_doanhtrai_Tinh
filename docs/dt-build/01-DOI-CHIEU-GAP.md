...
| DT-12 | Dashboard chỉ huy | `dt12-dashboard` (mới) + `dashboard`, `analytics`, `alerts` | 🟢 | Đã đóng (semantic/KPI + Data Mart + lineage + alert lifecycle + what-if) |
...
| DT-12 | **PASS** | 2026-09-09 | (đang commit) | Module MỚI `dt12-dashboard`: 23 bảng + migration reversible `1753000046000` + 30 API (`/api/v1/dt12/*`). **Lớp semantic/KPI** đọc SNAPSHOT CHUẨN của DT-04/06/08/09/10 (chỉ đọc — bất biến) qua `semantic-sources.ts`: 7 semantic tách bạch SEM-HC / HC-AVAILABLE (HC − hold EXCLUSIVE) / RESERVE-SSCĐ (hold SSCĐ) / PC-SCD / NC / SUPPLY-REQUIRED / GAP (TC-003). `kpi_definition` + `kpi_formula_version` + `kpi_threshold` + `metric_instance` (as_of_time + `lineage_json` + `metric_hash` tất định + freshness). **Freshness** tất định: so `source_version` metric với phiên bản nguồn hiện tại ⇒ STALE khi nguồn mới hơn (TC-006). **Không tạo "số Dashboard" độc lập** — mọi KPI có lineage về snapshot (AC-15/TC-015), drill-down KPI→dòng nguồn khớp tổng (TC-001). **Data Mart star schema**: dim_time/material/org/location/mission/quality + fact_inventory/requirement/balance/count + `data_mart_refresh` (rebuild từ snapshot TRONG transaction + phát **outbox** `DATA_MART_REFRESHED`, dẫn xuất — không sửa tay) + `/data-mart/freshness`. **Alert** cấu hình `alert_rule` + `alert_instance` lifecycle OPEN→ACK→RESOLVED (RESOLVED bất biến) + SLA due_at + gom trùng dedupe (evaluate KPI vs ngưỡng → severity — TC-012). **What-if cách ly**: `decision_session`/option/criterion/score/record — chấm điểm chuẩn hóa+trọng số KHÔNG ghi ngược metric/nguồn vận hành (TC-008). Bản đồ vật chất `@Scoped` theo data-scope (TC-010). **Test:** 21 unit + **7 integration** (DB thật — TC-DT12-001/003/006/008/012/015 + Data Mart) + **1 E2E** Playwright (chuỗi đầy đủ, PASS backend thật 3099). **Migration + seed (7 KPI + 2 alert_rule + nguồn demo) áp DB 5435 & 5436.** **Webapp** CommandDashboardPage `/dt12/command` phủ SCR-DT12-01..08 (tổng quan KPI+freshness, semantic explorer, drill-down, cảnh báo lifecycle, refresh Data Mart, what-if, quyết định, bản đồ). |
| Hardening (Core) | **PASS** | 2026-09-09 | (đang commit) | Tích hợp & nghiệm thu tổng thể. **SYS-BR-08**: khép rò rỉ list/search chéo đơn vị — thêm `common/scope/scope-query.ts` (`applyOrgScope`/`applyJsonOrgScope`/`assertReadScope`) áp vào service list/read DT-04…12 + `@Scoped('organization')`/`@CurrentUser` controller ⇒ 403 `NO_PERMISSION_SCOPE` đồng nhất (scope-enforcement.int-spec). **Bất biến SYS-BR-01…08** có bằng chứng `common/invariants/sys-br.spec.ts` (BR-02/03/04/05/07 + tái lập input/output_hash TC-HD-003). **Chuỗi vàng** `seed:golden-chain` (DT-01→DT-12 idempotent) + `golden-chain.int-spec.ts` (trace chứng từ→movement & lineage biểu→báo cáo không mồ côi, không tồn âm). **Quan trắc §5**: health tổng hợp `/health`(+`/live`/`/ready`) DB/PostGIS/MinIO/outbox; Prometheus `/metrics` (histogram latency + gauge outbox_backlog); log correlation-id; **rate-limit** throttler + **helmet**. **DQ §6**: `/data-quality/system` (tồn âm/dataset lệch/legacy norm/outbox lỗi) + unit test. **Contract** OpenAPI baseline cập nhật 425 path (additive, TC-HD-008). **Test:** `npm test` **287 PASS / 28 suite**. Chi tiết: `docs/dt-build/99-HARDENING-REPORT.md`. Hoãn (§7): load-at-scale, backup mã hóa, di trú M01…M15. |

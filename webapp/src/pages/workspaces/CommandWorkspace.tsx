...
// Workspace CHỈ HUY CẤP TỈNH — ra quyết định: KPI toàn tỉnh, toàn cảnh điều hành (M23),
// bản đồ, cơ cấu chất lượng và vấn đề cần xử lý. Có chế độ Thời bình/SSCĐ/Mô phỏng.
export function CommandWorkspace({ fullName }: { fullName?: string }) {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Chỉ huy cấp tỉnh · Trung tâm điều hành"
        title={`Trung tâm chỉ huy${fullName ? ` — ${fullName}` : ''}`}
        description={ROLE_TAGLINE.PROVINCIAL_COMMAND}
        actions={
      ...
      <QuickActions actions={quickActionsFor('command')} />
